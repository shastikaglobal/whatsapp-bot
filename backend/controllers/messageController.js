...
export const getMessages = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT * FROM messages WHERE customer_id = ? ORDER BY timestamp ASC', [conversationId]);
    res.json(rows);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
export const sendMessage = async (req, res) => {
  try {
    ...
    await pool.query('UPDATE conversations SET updated_at = CURRENT_TIMESTAMP WHERE customer_id = ?', [customerId]);
...
export const toggleTakeover = async (req, res) => {
  try {
    ...
    await pool.query('UPDATE conversations SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE customer_id = ?', [status, customerId]);
